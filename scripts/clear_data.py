...
class DataProcessor:
    ...
    def clear_data(self):
        ...
        df_all.drop(columns=self.drop_cols, inplace=True)

        df_all = out_schema(df_all)
    ...
    @pa.check_io(out=out_schema)
    def get_clean_train(self):
        return self._train

    @pa.check_io(out=out_schema)
    def get_clean_test(self):
        return self._test

    def get_data(self):
        X_train = StandardScaler().fit_transform(self._train.drop(columns=self.drop_cols))
        y_train = self._train['Survived'].values
        X_test = StandardScaler().fit_transform(self._test.drop(columns=self.drop_cols))
